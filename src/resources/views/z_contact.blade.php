...
<body>
    ...
    <div class="inputItems">
        <form class="form" action="{{ url('/z/contacts/confirm') }}" method="POST">
            @csrf
            <div class="inputRow">
            ...
            <div class="inputRow">
                ...
                <div class="inputArea">
                    <input class="input-gender" type="radio" id="male" name="gender" value="male" {{ (old('gender', $contact['gender'] ?? '') == 'male') ? 'checked' : '' }}>
                    <label for="male">男性</label><br>
                    
                    <input class="input-gender" type="radio" id="female" name="gender" value="female" {{ (old('gender', $contact['gender'] ?? '') == 'female') ? 'checked' : '' }} >
                    <label for="female">女性</label><br>
                    
                    <input class="input-gender" type="radio" id="otherGender" name="gender" value="otherGender" {{ (old('gender', $contact['gender'] ?? '') == 'otherGender') ? 'checked' : '' }} >
            ...
            <div class="inputRow">
                ...
                <div class="inputArea">
                    <select class="option-query" id="querySelect" name="content" >
                        ...
                        <option value="delivery" {{ old('content', $contact['content'] ?? '') == 'delivery' ? 'selected' : '' }}>商品のお届けについて</option>
                        <option value="exchange" {{ old('content', $contact['content'] ?? '') == 'exchange' ? 'selected' : '' }}>商品の交換について</option>
                        <option value="trouble" {{ old('content', $contact['content'] ?? '') == 'trouble' ? 'selected' : '' }}>商品トラブル</option>
                        <option value="question" {{ old('content', $contact['content'] ?? '') == 'question' ? 'selected' : '' }}>ショップへのお問い合わせ</option>
                        <option value="otherQuery" {{ old('content', $contact['content'] ?? '') == 'otherQuery' ? 'selected' : '' }}>その他</option>
                    </select>
                </div>
            </div>  
            <div class="form__error">
